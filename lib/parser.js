...
var AssignedEntity = function (resource, templateId) {
    ...
    if (templateId && _.contains(templateId, '*2.16.840.1.113883.10.20.22.4.87')) {
        ...
        this._self = {
            id: function (node) {
                if (node.attributes.extension && node.attributes.root) {
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.1.28': // Problem observation
            if (isInContextOf('2.16.840.1.113883.10.20.1.27')) { //Problem act
                ...
                condition = getResource('Condition', {
                    ...
                    'patient': encounter.patient.reference
                });
        ...
        case '2.16.840.1.113883.3.62.3.17.1': //Goal
            if (isInContextOf('2.16.840.1.113883.10.20.1.10')) {
                ...
                ensureProperty.call(carePlan, 'goal', true).push({
                    'reference': goal.id
                });
...
var Supply = function (resource) {
    ...
    this.templateId = function (node) {
        ...
        switch (node.attributes.root) {
        case '2.16.840.1.113883.10.20.1.34': //Supply Activity -> MedicationDispense

            patient = findPatient(proto.bundle);
            var medicationDispense = getResource('MedicationDispense', {
                'id': 'MedicationDispense/' + (serial++).toString()
            });
            store2bundle(medicationDispense, patient.id);

            this._self = {

                statusCode: function (node) {
                    // TODO Recode?
                    medicationDispense.status = node.attributes.code;
                },

                effectiveTime: function (node) {
                    var subType = node.attributes['xsi:type'];
                    ensureProperty.call(ensureProperty.call(medicationDispense, 'dispense'), 'scheduleDateTime');
                    medicationDispense.dispense.scheduleDateTime = dateFix(node.attributes.value);
                },

                repeatNumber: function (node) {
                    ensureProperty.call(medicationDispense, 'dispense').numberOfRepeatsAllowed = Number(node.attributes.value);
                },

                quantity: function (node) {
                    ensureProperty.call(medicationDispense, 'dispense').quantity = {
                        'value': Number(node.attributes.value)
                    };
                },

                product: function (node) {
                    proto.control.push(new Triplet(node, new Product(medicationDispense)));
                },
                ...
                author: function (node) {
                    proto.control.push(new Triplet(node, new Author(medicationDispense)));
                },
...
var Act = function (resource) {
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.1.27': //Problem act
            ...
            store2bundle(condition, patient.reference);
...
var Entry = function (resource) {
    ...
    this.act = function (node) {
        ...
        } else if (isInContextOf('2.16.840.1.113883.10.20.1.11' /*Problem Section*/ )) {
